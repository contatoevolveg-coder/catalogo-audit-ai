...
def render(api_url: str, headers: dict) -> None:
    st.markdown("### 🛒 Publicação de Anúncios no Mercado Livre")
    st.markdown("""
    Publique de forma síncrona anúncios aprovados diretamente no Mercado Livre utilizando 
    ...
    if st.button("🔍 Sugerir Categorias"):
        with st.spinner("Buscando sugestões de categoria no Mercado Livre..."):
            try:
                ...
                pred_url = f"{api_url}/marketplace-integrations/mercado-livre/category-suggestions?title={title_for_prediction}"
    ...
    category_id = st.text_input(
        "ID da Categoria (MLBxxxxx):",
        value=category_id,
        help="Insira o código de categoria oficial do Mercado Livre. Ex.: MLB1051"
    )

    # 3. Seleção de Credencial ML
    st.markdown("#### 🔑 Selecionar Credencial do Mercado Livre")
    try:
    ...
    # Filtra apenas provedor mercado_livre
    ml_creds = [c for c in all_creds if c["provider"] == "mercado_livre"]
    ...
    if st.button("Confirmo e Desejo Publicar no Mercado Livre", type="primary"):
        if not category_id.strip():
            ...
            with st.spinner("Enviando dados do anúncio ao Mercado Livre de forma síncrona..."):
                try:
                    r_pub = requests.post(
                        f"{api_url}/marketplace-integrations/mercado-livre/products/{product_id}/publish",
                        json=payload,
                        headers=headers
                    )
                    ...
                    if r_pub.status_code == 200:
                        res_pub = r_pub.json()
                        st.success(f"🎉 Anúncio publicado com sucesso! Item ID no Mercado Livre: **{res_pub['marketplace_item_id']}**")
                        st.rerun()
                    elif r_pub.status_code == 502:
                        # Falha de validação do próprio ML
                        error_detail = r_pub.json().get("detail", r_pub.text)
                        st.error(f"❌ Falha de validação da API do Mercado Livre: {error_detail}")
    ...
    try:
        r_hist = requests.get(
            f"{api_url}/marketplace-integrations/mercado-livre/products/{product_id}/publications",
            headers=headers
        )
    ...
    if not history:
        ...
        for h in history:
            hist_data.append({
                ...
                "Categoria ML": h["category_id"],
                "Status": h["status"],
                "ID Item ML": h.get("marketplace_item_id") or "N/A",
