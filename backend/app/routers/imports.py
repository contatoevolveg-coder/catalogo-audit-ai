...
from backend.app.database import get_db, ImportBatch, ImportRow
...
@router.get("/template")
def download_template(marketplace: str = "mercado_livre"):
...
def get_row(batch_id: int, row_id: int, db: Session = Depends(get_db)):
...
def confirm_import(batch_id: int, db: Session = Depends(get_db)):
    """Confirma a importação do lote, criando Products somente a partir de linhas válidas."""
    return import_service.confirm_import(batch_id, db)
